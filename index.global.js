 ...
 */`;
  ...
  var HomeScene = class _HomeScene {
            ...
            switch (name) {
              ...
              case "CCWData": {
                ...
                patch(
                  ...
                  (_getValueFromProject) => {
                    return async function(name2) {
                      const newValue = await _getValueFromProject.call(this, name2);
                      state_default.ccwdata.project.set(name2, newValue);
                      const possiblyModifiedValue = state_default.ccwdata.project.get(name2);
                ...
                patch(
                  ...
                  "_setValueToProject",
                  (_setValueToProject) => {
                    return async function(name2, value) {
                      state_default.ccwdata.project.set(name2, value);
                      return await _setValueToProject.call(
                        this,
                        name2,
                        state_default.ccwdata.project.get(name2)
                      );
                    };
                  }
                );
                patch(extensionObject, "_getValueFromUser", (_getValueFromUser) => {
                  return async function(name2) {
                    const newValue = await _getValueFromUser.call(this, name2);
                    state_default.ccwdata.user.set(name2, newValue);
                    const possiblyModifiedValue = state_default.ccwdata.user.get(name2);
                    if (possiblyModifiedValue !== newValue) {
                ...
                patch(extensionObject, "_setValueToUser", (_setValueToUser) => {
                  return async function(name2, value) {
                    state_default.ccwdata.user.set(name2, value);
                    return await _setValueToUser.call(
                      this,
                      name2,
                      state_default.ccwdata.user.get(name2)
                    );
                  };
                });
                break;
    ...
    renderFeatureList() {
      ...
      const features = _HomeScene.orderBy(this.featureList, [
        "\u{1F4DD} \u4F5C\u54C1\u6570\u636E",
        "\u{1F329}\uFE0F \u4E91\u6570\u636E",
        "\u{1F3AE} MMO \u6846\u67B6",
        "\u{1F3C6} \u6210\u5C31\u76F8\u5173\u529F\u80FD",
